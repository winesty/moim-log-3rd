...
## 시트에서 사람·만남 가져오기 (`/people/import`)
예전 시트(mtgCard: 한 행 = 한 사람과의 한 번의 만남)를 사람 / 모임 / 이야기로 옮깁니다.
1. 시트를 CSV로 내려받아 `/people/import`에서 업로드 → **미리보기** (이때는 아무것도 저장하지 않음)
2. 숫자 검증(원본 행 수 = 가져올 행 + 건너뛴 행, 이야기 글자 일치 등)이 모두 통과해야 "가져오기" 버튼이 켜집니다.
3. 가져온 내역마다 **되돌리기**가 있어, 그 가져오기로 들어온 것만 한 번에 지울 수 있습니다.

- 사람을 가리키는 열쇠는 자동으로 붙는 `id`입니다. 예전 관리번호(`이름-날짜`)는 `legacyMgmtNo`, 관리번호의 날짜는 `firstMetDate`(최초 만난 일자)로 보관합니다.
- 같은 이름이 둘 이상이면 화면에서 `김상규 (2017-02-07)`처럼 최초 만난 일자를 붙여 구분해 보여줍니다.
- Title/Age/School → 사람 정보, Career/주요 Network/Family/Hobby/기타 → 그 행의 날짜에 붙는 이야기(각각 기본 카테고리 `#커리어 #주요Network #가족관계 #취미 #기타`).
- 같은 파일을 다시 올려도 이미 가져온 행(No.)은 건너뛰므로 중복이 생기지 않습니다.
- 앱에 이미 같은 이름이 있어도 이름만으로 합치지 않고 새 사람으로 가져옵니다(미리보기에서 알려줍니다).
