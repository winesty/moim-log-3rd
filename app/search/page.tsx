          ...
          return (
            <div key={m.id} className="bg-white border border-[#ddd8ca] rounded-xl p-4">
              ...
              <div className="flex flex-col gap-1 mb-2">
                {m.stops.map((s) => (
                  <p key={s.id} className="text-sm text-[#7a7768]">
                    <span className="text-xs text-[#a09c8c]">{s.label}</span>{" "}
                    {s.place ? (
                      <Link href={`/places/${s.place.id}`} className="text-[#b4622f]">
                        {s.place.name}
                      </Link>
                    ) : (
                      "장소 미상"
                    )}
                    {[s.place?.city, s.place?.gu].filter(Boolean).length > 0 && ` · ${[s.place?.city, s.place?.gu].filter(Boolean).join(" ")}`}
                    {s.orderedItems && s.orderedItems.length > 0 && (
                      <span className="text-xs text-[#a09c8c]"> · 주문: {s.orderedItems.map((it) => `${it.name} x${it.quantity}`).join(", ")}</span>
                    )}
                  </p>
                ))}
              </div>
